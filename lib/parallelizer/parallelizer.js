...
module.exports = function () {
    ...
    function forkRunner(options) {
        ...
        _argv.forEach(function (val) {

            _newArgs.push(val);

            if (val.includes(PATH_TO_LIB_RUNNER)) {
                _newArgs.push(options.feature);
            }
        });

        if (options.feature.length > 0) {
            runnerIndex++;
            runner = ChildProcess.fork(path.join(__dirname, '../..' + PATH_TO_LIB_RUNNER), _newArgs);
        }
